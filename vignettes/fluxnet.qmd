...
```{r}
#| label: setup
# Create temp dir for this vignette to use
tmp <- withr::local_tempdir(pattern = "fluxnet_vignette")
```
...
```{r}
#| echo: false
list <- flux_listall(cache_dir = fs::path(tmp, "cache"))
```
...
```{r}
#| echo: false
# Actually download a sample of 5 sites
set.seed(123)
list_sample <- slice_sample(list, n = 5)
flux_download(list_sample, download_dir = fs::path(tmp, "fluxnet"))
```
...
# Actually extract annual and monthly data
flux_extract(zip_dir = fs::path(tmp, "fluxnet"), resolutions = c("y", "m"))
```
...
```{r}
#| echo: false
manifest <- flux_discover_files(data_dir = fs::path(tmp, "fluxnet", "unzipped"))
manifest
```
...
flagged_nee %>% filter(qc_flagged) %>% select(qc_flagged, p_gapfilled, NEE_VUT_REF_QC, everything())
```
...
flagged_2 %>% filter(qc_flagged) %>% select(qc_flagged, p_gapfilled, NEE_VUT_REF_QC, TA_F_QC, everything())
```
